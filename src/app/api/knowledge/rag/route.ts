...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    recordPerformance("/api/knowledge/rag", "POST", Date.now() - startTime, 400);
    return NextResponse.json(
  ...
  try {
    const result = await ragService.generateWithContext(body.query, {
      limit: body.limit,
      minSimilarity: body.minSimilarity,
    });
    recordPerformance("/api/knowledge/rag", "POST", Date.now() - startTime, 200);
    return NextResponse.json(result, { headers: rateLimitHeaders });
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    recordPerformance("/api/knowledge/rag", "POST", Date.now() - startTime, 500, undefined, msg);
